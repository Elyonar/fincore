 ...
 *
 * <p>The mapping that matters is the last one. **An unknown outcome is a 5xx, never a
 * success-shaped 202**, because under Core's own retry rule a 5xx obliges the caller to retry the
 * same idempotency key — and that retry is what eventually resolves the saga. A 202 would invite a
 * teller to record "submitted" and move on, telling a customer the transfer went through while the
 * platform does not know whether it did.
 */
...
public class ApiErrors {
    ...
    /** Deliberately body-less: naming the permission that would have worked is a map for a prober. */
     ...
     *
     * <p>503 rather than 500: the caller's correct response is to retry the same key, and 503 is
     * the status that says "ask again" without claiming anything about what happened. The
     * transaction id travels with it so the caller can poll {@code GET /v1/transactions/{id}}
     * instead of mutating to find out.
     */

...
public interface CustomerEligibility {
    ...
    /**
     * The KYC tiers this institution recognises, by code.
     *
     * <p>On this port because the tier's authority is the customer service, and the caller that
     * needs it is Core's pricing surface: a limit rule names a tier, and one naming a tier nobody
     * can hold stores cleanly and then never matches — a configured ceiling that behaves as a
     * blanket refusal, because the evaluator denies by default.
     *
     * <p>The product service cannot ask this itself. It would mean Product calling Customer, the
     * edge ADR 0020 exists to prevent. Core can see both, which is why the check sits with it —
     * the same reason Core, not Product, checks that a fee rule names one of its own accounts.
     *
     * <p>Empty means the institution has defined none, which is a provisioning gap rather than a
     * statement that no tier is valid. A caller must not read it as "refuse everything".
     */
    java.util.Set<String> recognisedTiers(UUID tenantId);

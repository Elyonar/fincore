 ...
 * and for the same reason: an endpoint nobody remembered to guard is closed rather than open. Doing
 * it per module would mean three implementations of one rule, and two of them would eventually
 ...
 * the same choice the ledger made and the same one Core already makes for a customer belonging to
 * another tenant.
 *
...
public class TenantGate extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            // An open path — health, docs, the service identity. The identity filter already let it
            // through, and a page with no caller has no tenant to check.
            chain.doFilter(request, response);
            return;
        }

        if (!tenants.isActive(tenantId)) {
            // Logged with the tenant, answered without it. An operator needs to know which
            // provisioning went wrong; the caller does not.

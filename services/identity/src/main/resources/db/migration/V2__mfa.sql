...
ALTER TABLE auth.mfa_enrollments FORCE ROW LEVEL SECURITY;
CREATE POLICY mfa_enrollments_tenant ON auth.mfa_enrollments
    USING (tenant_id = auth.current_tenant());
GRANT SELECT, INSERT, UPDATE, DELETE ON auth.mfa_enrollments TO identity_app;
...
CREATE INDEX mfa_recovery_by_user ON auth.mfa_recovery_codes (tenant_id, user_id);
...
CREATE POLICY mfa_recovery_tenant ON auth.mfa_recovery_codes
    USING (tenant_id = auth.current_tenant());
GRANT SELECT, INSERT, UPDATE, DELETE ON auth.mfa_recovery_codes TO identity_app;

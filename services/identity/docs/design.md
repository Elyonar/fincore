...
| D14 | MFA (TOTP) and step-up — **built** | TOTP enrolment/activation/verify, one-time recovery codes, step-up (`acr`/`auth_time`), and disable. Login returns `MFA_REQUIRED` as an `ACTION_REQUIRED` step in the same flow — no phase-1 shape changed to admit it. TOTP is RFC 6238 (HMAC-SHA1 from JCE, no invented crypto); the secret is encrypted at rest (AES-GCM, `SecretCipher`, key by reference) and recovery codes are stored only as digests. SMS OTP is the delivery-gated sibling (D18). |
| D17 | The full auth surface is authored as a contract, not generated | A banking core's auth API is reviewed whole, so `static/openapi.yaml` is the authoritative OpenAPI 3.1 spec — every endpoint, `x-status: built|planned`. Swagger UI at `/docs` renders it; the generated `/v3/api-docs` still reflects live controllers, so intent and reality sit side by side. |
| D18 | Email/phone verification, SMS OTP and self-service password reset are delivery-gated | Fully specified in `openapi.yaml` and designed here, implemented up to the point of delivery. The platform's messaging connector does not exist (notification senders are log adapters by decision), so a flow that sends a code to a person is `planned` until it does; administrator-initiated reset (built via the directory) covers the gap. Stated, not hidden. |

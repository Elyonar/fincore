...
public class ProductController {
    ...
    /**
     * One product by its code, or 404.
     *
     * <p>Added for the extraction (ADR 0020). Core confirms a product exists before opening an
     * account under it, and in process that was a one-line port call; over the wire the only
     * alternative was fetching the whole catalogue and filtering it, which is a list scan per
     * account opened and gets slower exactly as an institution grows.
     *
     * <p>404 rather than an empty body, because "no such product" is the whole answer the caller
     * needs and a 200 with nothing in it invites somebody to check the wrong field.
     */
    @GetMapping("/{code}")
    public ResponseEntity<ProductRecords.Product> byCode(@PathVariable String code) {
        var identity = Authorization.require("products:read");
        return products.list(identity.tenantId()).stream()
                .filter(one -> one.code().equalsIgnoreCase(code))
                .findFirst()

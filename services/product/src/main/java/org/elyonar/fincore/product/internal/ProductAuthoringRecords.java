...
import org.elyonar.fincore.product.api.ProductErrorReason;
...
public class ProductAuthoringRecords implements ProductAuthoring {
    ...
    /**
     * The channels the money path actually sends, mirrored here deliberately.
     *
     * <p>The set lives in Orchestration's transfer surface (ADR 0012), which Product may not
     * import. It stays a constant because a channel is code: adding `USSD` to a table would not
     * make the platform send over USSD, so a rule naming one is a rule that can never match, and
     * the evaluator denies by default.
     *
     * <p><strong>The KYC tier set that used to sit beside this is gone.</strong> It named the CBN's
     * three, which made the platform wrong for every institution outside one jurisdiction — and it
     * contradicted the customer service, where the tier has always been free text. An institution
     * could put somebody on a fourth tier and then be unable to price it: the tier existed on the
     * person and could not exist on the rule governing what they may move, so every transaction
     * under it was refused.
     *
     * <p>A tier is now the institution's vocabulary, in {@code customer.kyc_tiers}. This service
     * does not check a rule against that list and must not: it would mean Product calling Customer,
     * an edge ADR 0020 exists to prevent. The shape is still constrained — by the database, and by
     * the check below — so a rule cannot carry an empty or untypeable tier. Whether the tier is one
     * the institution recognises is a question for the surface that can see both, which is Core.
     */
    private static final Set<String> CHANNELS = Set.of("TELLER", "API");

    /** A tier has to be a usable handle even though its vocabulary is the tenant's. */
    private static final java.util.regex.Pattern TIER_HANDLE =
    ...
    private static void checkLimitShape(ProductAuthoring.LimitRule rule) {
        if (rule.kycTier() == null || !TIER_HANDLE.matcher(rule.kycTier()).matches()) {
            throw new RulesInvalid(
                    ProductErrorReason.UNKNOWN_KYC_TIER,
                    Map.of(
                            "kycTier", String.valueOf(rule.kycTier()),
                            "expects", "2 to 32 characters of A-Z, 0-9 or underscore"));
        }

 ...
 *
 * <p>{@code customer_app} serves every request, always scoped to one tenant by {@code SET LOCAL}.
 * There is no worker identity because there is no background work here — nothing is claimed from a
 * queue and nothing is drained. This service answers questions about people.
 *
 ...
 *
 * <p>The bean names are the ones {@code CustomerBeans} already declared when this was a module
...
public class DataSources {
    ...
    @Bean(CustomerBeans.DATA_SOURCE)
    @ConfigurationProperties("fincore.customer.datasource.app")
    ...
    @Bean(CustomerBeans.JDBC)
    public JdbcTemplate customerJdbcTemplate(
            @Qualifier(CustomerBeans.DATA_SOURCE) HikariDataSource customerDataSource) {
        return new JdbcTemplate(customerDataSource);
    }

    @Bean(CustomerBeans.TRANSACTION_MANAGER)

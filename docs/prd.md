...
- **Decisioning spectrum, one model:** the approval chain is amount-tiered *from zero* — a tier may require no human approval (fintech instant lending under a configured ceiling; rules and scores advise per §3.3, a deterministic policy decides), one approval (a solo lender operating under their own moneylender licence — the institution-wide maker-checker list in §6.3 does not include loan approval, and a one-person institution is a supported tenant shape), or an N-approval chain (MFB credit committees). Every tier's decision is attributed and audited identically.
- **Early settlement:** payoff quote as of a value date; unearned-interest rebate on flat-rate loans and configurable prepayment fee on declining-balance — the total-cost disclosure below must survive an early exit honestly.
- **Top-up and refinance:** a new loan whose disbursement settles a predecessor's payoff, linked for bureau reporting and portfolio history; never an in-place edit of the original schedule.
- **Guarantors and collateral:** guarantor undertakings (individuals or groups) with their own exposure view; lien on savings implemented as a Ledger hold on the pledged account (the hold machinery is exactly this); movable-collateral registration via a National Collateral Registry connector hook; collateral release on closure is part of the closure workflow, not an afterthought.
- **Credit life insurance:** premium as a disbursement-time deduction per product configuration, remitted to the insurer as a partner settlement; claims workflow deferred to the insurer relationship.
- **Collections rails:** repayment by direct-debit e-mandate and, for banks, GSI (Global Standing Instruction) recovery via the NIBSS connector; payroll deduction files for employer-channel lending. Lending owns *what is due*; connectors own *how it is collected*.
- **Disclosure & conduct pack (country-pack material, §3.1):** total cost of credit / effective annual rate on the offer, FCCPC digital-lending registration data points for fintech tenants, cooling-off where a framework requires it. Formats are country-pack configuration, never core code.

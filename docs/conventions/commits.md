...
| `core/orchestration` · `core/organization` · `core/admin` | the modules of services/core — **one scope per module, not one per deployable**, because the money-path merge gate attaches to `core/orchestration` alone and the scope is how a reviewer knows whether it applies. `customer` and `product` were scopes here until [ADR 0020](../adr/0020-customer-and-product-become-deployables.md); they are deployables now, so their scope is the bare service name |
